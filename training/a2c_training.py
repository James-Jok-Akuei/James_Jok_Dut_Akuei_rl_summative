...
def run(device: str = "auto", base_seed: int = 242, limit_runs: int | None = None, timesteps_scale: float = 1.0):
    from training.experiment_configs import A2C_SWEEP
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train A2C on the Emergency Medical Drone Delivery environment.")
    parser.add_argument("--device", default="auto", help="Torch device to use, for example cpu, cuda, or auto.")
    parser.add_argument("--base-seed", type=int, default=242, help="Base random seed for the sweep.")
